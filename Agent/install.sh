...
sudo mkdir -p /etc/ASRS/.config  2>/dev/null
cd /etc/ASRS/.config && sudo mkdir SSH_config rsync_config snapshot_config connection_config env_var 2>/dev/null
mkdir /etc/ASRS/.database 2>/dev/null
cd /etc/ASRS/.database && sudo mkdir status_logs snapshot_logs detection_marker rsync_logs 2>/dev/null
...
for log in "${logs[@]}"; do
    cd /etc/ASRS/.database/"$log" && touch "$log".json
done
for confe in "${confingu[@]}"; do
    cd /etc/ASRS/.config/"$confe" && touch "$confe".json
done
